...
if [[ "${I_APPROVE_FINAL_LIVE_STRATEGY_START:-}" != "YES" ]]; then
  echo "Refusing to run XRPUSDC final live strategy. Missing I_APPROVE_FINAL_LIVE_STRATEGY_START=YES."
  exit 1
fi
...
python scripts/check_config.py >/tmp/xrpusdc-pivot-bot-check-config.json
